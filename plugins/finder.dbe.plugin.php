...
function Select($arrValues, $selectName, $default, $formname, $nullEntry)
	{
		if ($formname != null)	$output = "<select name='".$selectName."' onchange=\"submitform('".$formname."')\">";
		else	$output = "<select name='".$selectName."'>";
		
		if ($default != null) $output = "".$output."<option value='".$default."' selected='selected'>".$arrValues[$default]."</option>";
		else $output = "".$output."<option value=''>".$nullEntry."</option>";
