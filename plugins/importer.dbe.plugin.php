        ...
        if ($_FILES['file']['size'] <= UPLOAD_MAX_FILE_SIZE) {
            if ($_FILES['file']['tmp_name'] != $empty) {
                ...
                if (move_uploaded_file($_FILES['file']['tmp_name'], $this -> path)) {
                    ...
                    $error = new error("Error opening file!");

                if ($this -> format == 'SQL')
                    $status = $this -> ImportSQL();
                else if ($this -> format == 'CSV')
                    $status = $this -> ImportCSV();
                else if ($this -> format == 'XML')
                    $status = $this -> ImportXML();
                ...
                if ($status == true) {
                    ...
                    if ($this -> table -> connection -> result_id !== true && $this -> table -> connection -> result_id !== false) {
                        ...
                        $list -> Show();
                    } else if ($this -> table -> connection -> result_id == true)
                        echo "<div><fieldset class='ok'>
                    <legend>Note</legend><p class='centered'>Executed Successfully</p></fieldset></div>";

                } else
                    echo "<div><fieldset class='error'>
                    <legend>Error</legend><p class='centered'>Document parsing failed!</p></fieldset></div>";
            }

        } else
            $error = new error("Maximum allowed size is " . UPLOAD_MAX_FILE_SIZE . " bytes!</p>Current file size is " . $_FILES['file']['size'] . "");
    }
    ...
    public function ImportXML()
    {
        ...
        if ($result == true) {
            ...
            for ($y = 0; $y < $xml['nodelist']['table'] -> length; $y++) {
                ...
                if ($insert != null)
                    $this -> query = $this -> query . "; " . $insert;

                if ($this -> table -> field[$colcount - 1] != null)
                    $rowcount++;
            }
