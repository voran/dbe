...
class indexer extends dbeinterface implements plugin
{
    ...
    function __construct($table)
    {
        ...
        if ($this -> table -> connection -> result_id == null) {
            ...
            if ($this -> table -> connection -> keyword != null)
                $query = $this -> table -> connection -> KeywordScan($this -> table);
            else
                $query = $this -> table -> GenerateSelect();
    ...
    function Show()
    {
        ...
        if ($this -> table -> rowcount < 1)
            echo "<div class='heading'>Note</div>No Results to Display";
        else {
