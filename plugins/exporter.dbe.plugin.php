	...
	
{
	...
	if ($this->format != null) 
	{
		...
		if ($this->export == 1)
		{
			if ($this->format == 'XML') $this->ExportXML();

			else if ($this->format == 'CSV') $this->ExportCSV();

			else if ($this->format == 'SQL') $this->ExportSQL();
		...
		else $this->ShowAdditionalOptions();
	}
	echo "</fieldset></div>";
}
	...
	public function ExportXML()
	{
		...
		if ($this->DBE->db->name != null)	$element['db'] = $file->createElement($this->DBE->db->name);
		else	$element['db'] = $file->createElement("root");
