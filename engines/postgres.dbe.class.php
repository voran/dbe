...
class PostgresExplorer extends connection
{
    ...
    {
        $this -> link = @pg_connect("host=" . $server . "  user=" . $user . " password=" . $password . "");
    }
    ...
    {
        return pg_result($this -> result_id, $i);
    }
    ...
    {
        $this -> Execute("SELECT MAX(\"" . $table -> field -> pnum[0] -> name . "\") FROM \"" . $table -> name . "\"");
    }
    ...
    }//REQUIRES POSTGRES 7.2 or later
