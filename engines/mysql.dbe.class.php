...
class MysqlExplorer extends connection
{
    ...
    {
        return $this -> Execute("SELECT MAX(`" . $table -> pnum[0] -> name . "`) FROM `" . $this -> table -> name . "`");
    }
    ...
    {
        if ($query != $empty) {
            $this -> result_id = @mysql_query($query);
            // Perform Query
    ...
    public function FetchField($i)
    {
        ...
        if ($fieldobject -> not_null == 1)
            $field -> nullable = false;
        else
            $field -> nullable = true;

        if ($fieldobject -> primary_key == 1)
            $field -> primary = true;
        else
            $field -> primary = false;
