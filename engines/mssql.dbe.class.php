...
/*//////////////////////////////////////////////       ALIASES		/////////////////////////////////////////////////////////*/
public function Connect($server, $user, $password) 	{	$this->link = @mssql_connect($server, $user, $password);		}
public function FetchRow() 		     		{	return mssql_fetch_row($this->result_id);			}
public function NumFields()				{	return mssql_num_fields($this->result_id);			}
public function Result($i)				{	return mssql_result($this->result_id, $i, 0);			}
public function SelectDatabase($db)			{	$this->Execute("USE ".$db);					}
public function SelectDatabaseSchema()			{	return null;							}
public function GetMax($table)				{	$this->Execute("SELECT TOP 1 ".$table->pnum[0]->name." FROM ".$table->name." ORDER BY ".$table->pnum[0]->index." DESC");	}
public function ServerError($empty)			{	return mssql_get_last_message();				}
public function ConnectionError()			{	return mssql_get_last_message();				}
public function EscapeString($string)			{	return mysql_real_escape_string($string);			}
public function EncapsulateField($string)		{	return str_replace("'", "''", $string);				}
...
public function Execute($query)                                                           
	...
	if ($query != $empty)
		...
		{
			$this->result_id = mssql_query($query); // Perform Query
