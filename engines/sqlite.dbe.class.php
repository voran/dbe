...
/*//////////////////////////////////////////////       ALIASES		/////////////////////////////////////////////////////////*/
public function Connect($server, $user, $password) 	{	$this->link = @sqlite_open($server);				}
public function FetchRow() 		     		{	return sqlite_fetch_array($this->result_id, SQLITE_NUM);	}
public function NumFields()				{	return sqlite_num_fields($this->result_id);			}
public function SelectDatabase($db)			{	return null;							}
public function SelectDatabaseSchema()			{	return null;							}
public function GetMax($table)				{	return null;							}
public function ServerError($empty)			{	return "Error!";						}
public function ConnectionError()			{	return "Error!";						}
public function EscapeString($string)			{	return sqlite_escape_string($string);				}
...
public function Execute($query)                                                           
	...
	if ($query != $empty)
		...
		{
			$this->result_id = sqlite_query($query, $this->link, $errormsg); // Perform Query
	...
	{
		$limit = explode(',',LIMIT);
		return $query." LIMIT ".$limit[$this->limit];//OFFSET 0 , LIMIT x
	}
