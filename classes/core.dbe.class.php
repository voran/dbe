...
public function InitializeConnection($connection)
{
	if ($connection == null)
	{
		if  ($_SESSION['engine'] == 'mysql')	$this->connection = new MysqlExplorer();
		if  ($_SESSION['engine'] == 'pgsql')	$this->connection = new PostgresExplorer();
		if  ($_SESSION['engine'] == 'mssql')	$this->connection = new MssqlExplorer();
		if  ($_SESSION['engine'] == 'oci8') 	$this->connection = new OracleExplorer();
		if  ($_SESSION['engine'] == 'interbase') 	$this->connection = new FirebirdExplorer();
		if  ($_SESSION['engine'] == 'sqlite')		$this->connection = new SqliteExplorer();
	}
...
public function GetDatabases()
{
		...
		{	
			...
			if ($this->dbindex == null)	$this->dbindex = 0;
			...
			if ($this->connection->DatabaseScan() == null) $this->db->valid = 1;
			$this->connection->SelectDatabase($this->db->name);

		}

		if ($this->dbcount < 1) $this->db = new db(null, $this->connection, $x);
}
