...
public function CommonHiddenInputs($action = 'default', $query = null)
{
	...
	if ($action == 'default') $action = $this->action;
	...
	if ($query != null) $inputs .= "<input type='hidden' name='query' value='".$query."'/>";
...
public function GenerateSortButtons()
{
	for($x = 0; $x < $this->table->fieldcount; $x++)
	{
		...
		if ($this->table->connection->ordertype == 'ASC' && $this->table->connection->orderby == $x) $asc = ICON_ASC_SELECTED;
		else	$asc = ICON_ASC;
		if ($this->table->connection->ordertype == 'DESC' && $this->table->connection->orderby == $x) $desc = ICON_DESC_SELECTED;
		else	$desc = ICON_DESC;
...
else return"<input size='".EDIT_FIELD_SIZE."' name=\"".$x."\" value=\"".htmlspecialchars($this->table->field[$x]->value[0])."\" />";
...
public function ShowFormatOptions()
{
	...
	{
		$bt .= "<span>";
		if($f == $this->format) $bt .= "<input type='radio' name='format' value='".$f."' onclick=\"submitform('format')\" checked='checked'/>".$f;
		else $bt .="<input type='radio' name='format' value='".$f."' onclick=\"submitform('format')\"/>".$f;
		$bt .= "</span>";
	}
