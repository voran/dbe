...
function __construct()
{
	...
	if ($this->action == null) $this->action = DEFAULT_ACTION;
...
public function InitializeConnection()
{
	...
	if ($_SESSION['dbuser'] == null) $login = new login();	
...
public function LoadPlugins()
{
	...
	foreach (glob("./plugins/*.dbe.plugin.php") as $filename)
	{
		...
		if ($action == $this->action) $this->plugins[$i++] = new $plugin($this->table);
		else if (!in_array($action, $this->actions)) array_push($this->actions, $action);
	}
}
